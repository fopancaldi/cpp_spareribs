...
int main() {
    ...
    constexpr std::size_t simulations = 1 << 20, generator_seed = 0;
    ...
    curandGenerateNormal(u_generator, sim_pack1.u(), sim_pack1.len(), u_0,
    ...
    Rk4EvMap rk4_em(FhnNeuron{.epsilon = epsilon, .a = a}, float_type{});
    ...
    for (unsigned int steps_since_check = 0;; ++steps_since_check) {
        SimulationPack<float_type> const& past = steps_since_check % 2 == 0 ? sim_pack2 : sim_pack1;
        SimulationPack<float_type>& future = steps_since_check % 2 == 0 ? sim_pack1 : sim_pack2;
